...
# 对于 FCN-8s 而言,首先进行 pool4 feature 和 2 x upsampled feature 逐点相加,然后又进行 pool3 feature 和 2 x upsampled feature 逐点相加
with tf.variable_scope('vgg_16/fc8'):
    aux_logits_16s = slim.conv2d(pool4_feature, number_of_classes, [1, 1],
                                 activation_fn=None,
                                 weights_initializer=tf.zeros_initializer,
                                 scope='aux_logits_16s_conv_pool4')
    aux_logits_8s = slim.conv2d(pool3_feature, number_of_classes, [1, 1],
                                 activation_fn=None,
                                 weights_initializer=tf.zeros_initializer,
                                 scope='aux_logits_8s_conv_pool3')

# Perform the upsampling
upsample_filter_np_x2 = bilinear_upsample_weights(2,  # upsample_factor,
                                                  number_of_classes)
# step1: 16s upsampling
upsample_filter_tensor_x2_x16 = tf.Variable(upsample_filter_np_x2, name='vgg_16/fc8/t_conv_x2_x16')

upsampled_logits = tf.nn.conv2d_transpose(logits, upsample_filter_tensor_x2_x16,
...
#step2: 8s upsampling
upsample_filter_tensor_x2_x8 = tf.Variable(upsample_filter_np_x2, name='vgg_16/fc8/t_conv_x2_x8')

upsampled_logits = tf.nn.conv2d_transpose(logits, upsample_filter_tensor_x2_x8,
